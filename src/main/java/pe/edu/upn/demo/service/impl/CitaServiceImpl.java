...
public class CitaServiceImpl implements CitaService{
	...
	public ByteArrayInputStream exportAllData() throws Exception {
		...
		for (Cita cita : citas) {
			...
			row.createCell(3).setCellValue(cita.getServicio().getTrabajador().getNombre());
			row.createCell(4).setCellValue(cita.getFecha().toString());
			row.createCell(5).setCellValue(cita.getServicio().getHorario().getDescripcion());
